...
def decoding_batch(weights, is_multi_root, max_dependency_len, length_constraint_on_root, device):
    if device == 'cpu':
    ...
    inside_table = torch.zeros(batch_size, sentence_len*sentence_len*8, dtype=torch.double, device=device)
    inside_table.fill_(LOGZERO)

    (seed_spans, base_left_spans, base_right_spans, left_spans, right_spans,
     ijss, ikss, kjss, id_span_map, span_id_map) = constituent_indexes(sentence_len, is_multi_root, max_dependency_len, length_constraint_on_root)

    kbc = torch.zeros_like(inside_table, dtype=torch.long, device=device)
    ...
    for ij in ijss:
        ...
        if ij in left_spans:
            ...
            inside_table[:, ij] = torch.max(inside_table[:, ij], prob)
            ...
            inside_table[:, ij], kbc[:, ij] = probs.max(dim=1)
    ...
    best_score = torch.max(root_prob1, root_prob2)
    mask = torch.eq(best_score, root_prob1)

    root_id = torch.zeros((batch_size, 1), dtype=torch.long, device=device)
    root_id.fill_(id2)
    root_id[mask] = id1
    best_tree = back_trace_batch(device, kbc, root_id, sentence_len, is_multi_root, max_dependency_len,
                                 length_constraint_on_root)
    return best_score, best_tree


def back_trace_batch(device, kbc, root_id, sentence_len, is_multi_root, max_dependency_len, length_constraint_on_root):
    (seed_spans, base_left_spans, base_right_spans, left_spans, right_spans,
    ...
    in_tree = -torch.ones(batch_size, num_ctt, dtype=torch.long, device=device)
    trees = -torch.ones(batch_size, sentence_len, dtype=torch.long, device=device)

    for sent_id in range(batch_size):
        current_sent_root_id = root_id[sent_id][0].item()
